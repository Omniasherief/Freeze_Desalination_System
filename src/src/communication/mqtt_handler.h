...
// const char* MQTT_ = "192.168.1.51";  // change
...
// // MQTT broker configuration
// const char* MQTT_BROKER = "192.168.1.86";
// const int MQTT_PORT = 1883;
// const char* MQTT_CLIENT_ID = "ESP32_WaterSystem";
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
    ...
    // 1. Process System Mode Change Command (MANUAL/AUTO)
    if (t == "water/control/mode") {
        manual_mode_active = (msg != "AUTO");
        Serial.print("Mode changed: ");
        Serial.println(manual_mode_active ? "MANUAL" : "AUTO");

        // **Critical Fix:** Publish the new status immediately for Node-RED feedback
        publishSensor("water/system/mode", manual_mode_active ? "MANUAL" : "AUTO");

        return; // Exit to prevent processing as a manual control command
    }

    // 2. Block control commands if the system is in AUTO mode
    if (!manual_mode_active) return; // Only executes if MANUAL mode is active

    // 3. Process Manual Control Commands
    if (t == "water/control/pump1") { 
        pump1_state = (msg == "ON"); 
        updatePumps(); 
    }
    ...
    // Process Valve 1 (OPEN/CLOSE)
    else if (t == "water/control/valve1") {
        if (msg == "OPEN") {
            valve1_state = true;
            valve1_timer = millis();
        } else if (msg == "CLOSE") {
            valve1_state = false; // Fix: Immediate closing upon receiving CLOSE command
        }
        updateValves();
    }
    // Process Valve 2 (OPEN/CLOSE)
    else if (t == "water/control/valve2") {
        if (msg == "OPEN") {
            valve2_state = true;
            valve2_timer = millis();
        } else if (msg == "CLOSE") {
            valve2_state = false; // Fix: Immediate closing upon receiving CLOSE command
        }
        updateValves();
    }
}
...
// ------------------------
// Publish all sensor/status data periodically (called from loop)
// ------------------------
void publishAllMQTT() {
    char buf[32];
    publishSensor("water/LS1", LS1_state?"1":"0");
